...
$upload_url=$_REQUEST['upload_url']; $inputTitle=$_REQUEST['inputTitle']; $mytags=$_REQUEST['mytags'];
$descrip=$_REQUEST['descrip']; $userID=$_REQUEST['userid'];  $video_status=$_REQUEST['vstatus'];
...
$createdAt_convert=gmdate("Y-m-d", $createdAt);
if($entry_id!='')
{
  $upload_date=date("Y-m-d", $createdAt);
   $upload_entry="Insert into entry(entryid,name,thumbnail,longdescription,type,tag,status,ispremium,created_at,video_status)
   values('$entry_id',$name,'$thumbnailUrl',$description,'$type',$tags,'$status','0','$createdAt_convert','$video_status')";
