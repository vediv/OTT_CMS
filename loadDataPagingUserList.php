   ...
   <td width="15%" align="center">
    ...
    <td width="25%">
        <div class="col-sm-3 col-md-3 pull-right navbar-form" role="search">  
            ...
            <input class="form-control" size="30" onkeyup="SeachDataTable('loadDataPagingUserList.php','<?php echo $pagelimit;?>','<?php //echo $page; ?>','load','<?php echo $filtervalue; ?>')"  placeholder="Search Entries by name,email"  autocomplete="off" name='searchQuery' id='searchInput' class="searchInput" type="text" value="<?php echo $searchKeword; ?>">
            <div class="input-group-btn">
              <button class="enableOnInput btn btn-default" disabled='disabled' onclick="SearchDataTableValue('loadDataPagingUserList.php','<?php echo $pagelimit;?>','<?php //echo $page; ?>','load','<?php echo $filtervalue; ?>')" id='submitBtn' type="button" style="height: 30px;   padding: 4px 6px !important;"><i class="glyphicon glyphicon-search"></i></button>	
            </div>
            </div>
       </div>
    </td>
...
if($searchKeword!='')
{
    $query_search = " and (user_id LIKE '%" . $searchKeword . "%' or uemail LIKE '%" . $searchKeword . "%'or ulocation LIKE '%" . $searchKeword . "%'or ugender LIKE '%" . $searchKeword . "%')";
}
$reg_id_user =(isset($_POST['reg_id_user']))? $_POST['reg_id_user']: 0;
$queryUser='';
if($reg_id_user!=0)
{
    $queryUser = " and uid ='$reg_id_user'";
}
$filterQuery=$filtervalue==2?" where status!=2":" where status='".$filtervalue."'";
$query = "SELECT COUNT(uid) as num FROM user_registration $filterQuery $query_search $queryUser";
...
if($page) 
        ...
        $start = 0;
$sql="Select * from user_registration $filterQuery $query_search $queryUser order by added_date DESC  LIMIT $start, $limit";
...
?>    
    ...
    <?php
    if ($page == 0) $page = 1;
    $adjacent=3; $targetpage=''; $fromdate=''; $todate=''; $pageUrl='loadDataPagingUserList.php';
