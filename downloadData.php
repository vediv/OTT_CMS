...
<div class="modal-header">
    <button type="button" class="close" data-dismiss="modal">&times;</button>
    <h4 class="modal-title">Export Participant List</h4>
</div>
<div class="modal-body" style="border: 0px solid red; height: 220px; ">
  <div class="box">
    <div class="box-body" id="inner-content-div" style="border: 0px solid red;">
    <form name="myForm" action="operation.php?action=participantlist" method="post" onsubmit="return downloadParticipantList()">
     <div class="row" style="border:0px solid red;">
       <div class="col-md-12" style="text-align: center">
        <label for="from">From</label>
            <input type="text" id="fromDate" readonly name="fromDate" autocomplete="off"     />
    ...
    <button  type="button" id="pmypdf" class="btn btn-info" value=".pdf"  onclick="return downloadpdf()">
    <i class="fa fa fa-file-pdf-o" aria-hidden="true" style="padding: 0px 7px 0 0; color: rgb(221, 0, 0);"></i>Download PDF</button>
    </div>
        </div>
      </div>
...
function downloadpdf()
{
        ...
        var mypdf = document.getElementById("pmypdf").value;
        if(mypdf==".pdf")
        {
          window.open("operation.php?action=participantListPdf&fromDate="+fromDate+"&toDate="+toDate,'_blank');
         //window.location.href="operation.php?action=pdf&fromDate="+fromDate+
         return true;
        }
}
