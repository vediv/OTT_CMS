...
$id=$_REQUEST['id'];
$query1 = "SELECT par_id,name,email,menu_permission,operation_permission,other_permission FROM publisher where par_id='$id' ";
...
$other_permission=$value[0]['other_permission'];
$mpermission=explode(",",$menu_permission); $otherPermission=explode(",",$other_permission);
...
<div class="row">
<?php 
//if($menu_permission==''){ $menu_permission=0; }
//$get_main_menu_query = "SELECT mid,mname,menu_url,mparentid,multilevel,child_id,icon_class
//        FROM menus where mid IN($menu_permission) and  mparentid='0' and multilevel!='1' and mstatus='1'";
$get_main_menu_query = "SELECT mid,mname,menu_url,mparentid,multilevel,child_id,icon_class
        FROM menus where mparentid='0' and multilevel!='1' and mstatus='1'";
...
<label class="checkbox-inline" style="color: #3290D4;">
    ...
    <label class="checkbox-inline" style="color: #3290D4;">
     <input type="checkbox" name="action1[]" value="4" <?php if(in_array(4,$om)){ echo $chk="checked"; }  ?>> Delete
</label>
</p>
<?php 
if($other_permission!=''){
...
<label class="checkbox-inline" style="color: #3290D4;">
    <input type="checkbox" name="other_permission1[]" <?php if(in_array(5,$otherPermission)){ echo $chk="checked"; }  ?> value="5"> Plan 
</label>
<?php } if(in_array(6, $otherPermission)){    ?>   
    <label class="checkbox-inline" style="color: #3290D4;">
     <input type="checkbox" name="other_permission1[]" <?php if(in_array(6,$otherPermission)){ echo $chk="checked"; }  ?> value="6"> Advertisements 
</label>
<?php } ?>
</p>
<?php } ?>
    ...
    for (var i=0, n=checkboxes.length;i<n;i++) 
    {
      if (checkboxes[i].checked) 
      {
          ...
          vals +=checkboxes[i].value+',';
            ...
            var action = document.getElementsByName('other_permission1[]');  
               var val_other = "";
              for (var i=0, n=action.length;i<n;i++) 
              {
                if (action[i].checked) 
        ...
        method: 'POST',
        data:{ 'duser':duser,'demail':demail,'menuright':vals,'actionright':val,'other_permissions':other_permissions,'parid':parid,'action':"editdashboarduser"},
